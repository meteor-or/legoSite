...
window.addEventListener("scroll", function () {
  ...
  let productScroll =
    document.querySelector(".productWrap").offsetTop * 3 +
    document.querySelector(".productWrap").clientHeight;
...
const loop = () => {
  ...
  r++;
  html.style.setProperty("--changing", `hsla(${r / 1.5}, 100%, 50%, 0.999)`);
...
$.ajax({
  ...
  $("img[alt=promotion]").attr({ src: msg.documents.image_url });
});
...
$(window).resize(function () {
  ...
  if (width_size <= 1440) {
    alert("이 사이트는 1440px 이상 웹에 최적화 되어있습니다.");
  }
});
